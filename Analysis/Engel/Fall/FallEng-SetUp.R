...
eng_fall <- read.csv("./Data/dfo_camp_dat.csv") %>% mutate(v.t.s = paste(VESSEL,TRIP,SET, sep = ".")) %>%
...
eng_fall <- eng_fall %>% filter(data.series == "Engel" & season == "FALL"& is.na(bot.temp) == FALSE) %>%
  filter(year >= 1981) %>% 
  mutate(lat_dd = as.numeric(trimws(lat_dd)),
         long_dd = as.numeric(trimws(long_dd)))


call_fall <- read.csv("./Data/engel_call_dat.csv")
call_fall <- left_join(call_fall, 
                       eng_fall %>% select(v.t.s, lat_dd, long_dd, set.depth.max, bot.temp), by = c("v.t.s"))
call_fall <- call_fall %>% filter(data.series == "Engel" & season == "fall" & is.na(bot.temp) == FALSE) %>% 
...
full_fall <- left_join(full_fall, 
                       eng_fall %>% select(v.t.s, data.series, set.depth.max, bot.temp), by = c("v.t.s"))
full_fall <- full_fall %>% mutate(alt.name = PRED_COMM_NAME,
  ...
  filter(data.series == "Engel" & SEASON == "Fall" & is.na(length) == FALSE & length < 400 & is.na(bot.temp) == FALSE) %>%
  select(year, alt.name, content, pa, length, bot.temp, lat_dd, long_dd)

# Merge species called and full stomach contents
stom_fall <- rbind(call_fall, full_fall) %>% filter(year >= 1981)
...
trawlf <- eng_fall_good %>% rename(pa = trawl_pa) %>% select(year, pa, bot.temp)
...
AP_stof <- stom_fall_good %>% filter(alt.name == "American plaice") %>% select(year, pa, length, content, bot.temp)

# Set up different length bins
ac1 <- AC_stof %>% filter(length > 17 & length <= 45)
ac2 <- AC_stof %>% filter(length > 45) %>% filter(year != 1994)
