...
# Trawl data
eng_fall <- read.csv("./Data/camp_capelin_2J3KL_FULL.csv") %>% 
...
eng_fall <- eng_fall %>% rename(year = YEAR, trawl_pa = Capelin_PA) %>% 
...
cords <- data.frame(long = -eng_fall$long_dd, 
                    lat = eng_fall$lat_dd) 
...
AC_stof <- stom_fall_good %>% filter(alt.name == "Atlantic cod") %>% select(year, pa, empty, length, season, content, bot.temp) %>% mutate(geartype = "engel")
GH_stof <- stom_fall_good %>% filter(alt.name == "Greenland halibut") %>% select(year, pa, empty, length, season, content, bot.temp) %>% mutate(geartype = "engel")
AP_stof <- stom_fall_good %>% filter(alt.name == "American plaice") %>% select(year, pa, empty, length, season, content, bot.temp) %>% mutate(geartype = "engel")
