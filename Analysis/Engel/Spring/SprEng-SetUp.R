...
cords <- data.frame(long = -eng_spr$long_dd, 
                    lat = eng_spr$lat_dd) 
...
AC_stos <- stom_spr_good %>% filter(alt.name == "Atlantic cod") %>% select(year, pa, empty, length, season, content, bot.temp) %>% mutate(geartype = "engel")
GH_stos <- stom_spr_good %>% filter(alt.name == "Greenland halibut") %>% select(year, pa, empty, length, season, content, bot.temp) %>% mutate(geartype = "engel")
AP_stos <- stom_spr_good %>% filter(alt.name == "American plaice") %>% select(year, pa, empty, length, season, content, bot.temp) %>% mutate(geartype = "engel")
