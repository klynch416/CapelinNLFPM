...
# Trawl data
eng_spr <- read.csv("./Data/camp_capelin_2J3KL_FULL.csv") %>% rename(year = YEAR, trawl_pa = Capelin_PA) %>% 
...
eng_spr <- eng_spr %>% filter(data.series == "Engel" & 
                                season == "spring"& 
...
stom_spr <- rbind(call_spr, full_spr) %>% filter(year >= 1984)
...
trawls <- eng_spr_good %>% rename(pa = trawl_pa) %>% select(year, pa, bot.temp) %>% mutate(geartype = "engel")
AC_stos <- stom_spr_good %>% filter(alt.name == "Atlantic cod") %>% select(year, pa, length, season, empty, content, bot.temp) %>% mutate(geartype = "engel")
GH_stos <- stom_spr_good %>% filter(alt.name == "Greenland halibut") %>% select(year, pa, length, season, empty, content, bot.temp) %>% mutate(geartype = "engel")
AP_stos <- stom_spr_good %>% filter(alt.name == "American plaice") %>% select(year, pa, length, season, empty, content, bot.temp) %>% mutate(geartype = "engel")


# Set up different length bins
ac1 <- AC_stos %>% filter(length > 17 & length <= 45) %>% filter(year != 1995) # removed years with fewer than 10 overall observations
ac2 <- AC_stos %>% filter(length > 45) %>% filter(year != 1995)
ap1 <- AP_stos %>% filter(length > 29)
gh1 <- GH_stos %>% filter(length > 19 & length <= 40)
