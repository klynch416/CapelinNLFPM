...
eng_spr <- read.csv("./Data/dfo_camp_dat.csv") %>% mutate(v.t.s = paste(VESSEL,TRIP,SET, sep = ".")) %>%
  rename(year = YEAR, trawl_pa = Capelin_PA)
eng_spr <- left_join(eng_spr, 
                     setdet %>% select(v.t.s, set.depth.max, data.series, bot.temp), by = c("v.t.s"), multiple = "any")
eng_spr <- eng_spr %>% filter(data.series == "Engel" & season == "SPRING" & is.na(bot.temp) == FALSE) %>%
  filter(year >= 1981) %>% 
  mutate(lat_dd = as.numeric(trimws(lat_dd)),
         long_dd = as.numeric(trimws(long_dd)))


call_spr <- read.csv("./Data/engel_call_dat.csv")
call_spr <- left_join(call_spr, 
                      eng_spr %>% select(v.t.s, lat_dd, long_dd, set.depth.max, bot.temp), by = c("v.t.s"))
call_spr <- call_spr %>% filter(data.series == "Engel" & season == "spring" & is.na(bot.temp) == FALSE) %>%
  mutate(content = rep("called", length(data.series))) %>% 
  select(year, alt.name, content, pa, length, bot.temp, lat_dd, long_dd)
...
full_spr <- left_join(full_spr, 
                      eng_spr %>% select(v.t.s, data.series, set.depth.max, bot.temp), by = c("v.t.s"))
full_spr <- full_spr %>% mutate(alt.name = PRED_COMM_NAME,
  ...
  rename(year = YEAR, length = LENGTH) %>%
  filter(data.series == "Engel" & SEASON == "Spring" & is.na(length) == FALSE & length < 400 & is.na(bot.temp) == FALSE) %>%
  select(year, alt.name, content, pa, length, bot.temp, lat_dd, long_dd)

# Merge species called and full stomach contents
stom_spr <- rbind(call_spr, full_spr) %>% filter(year >= 1981)
...
cords <- data.frame(long = -eng_spr$long_dd, lat = eng_spr$lat_dd) 
anomaly_spdf <- sp::SpatialPointsDataFrame(cords, data.frame(pa = eng_spr$trawl_pa))
...
eng_spr_good <- eng_spr %>% filter(lat_dd %in% good_coords$lat_dd & long_dd %in% -good_coords$long_dd)
stom_spr_good <- stom_spr %>% filter(lat_dd %in% good_coords$lat_dd & long_dd %in% -good_coords$long_dd)


# Separate data by species
trawlf <- eng_spr_good %>% rename(pa = trawl_pa) %>% select(year, pa, bot.temp)
AC_stof <- stom_spr_good %>% filter(alt.name == "Atlantic cod") %>% select(year, pa, length, content, bot.temp) 
GH_stof <- stom_spr_good %>% filter(alt.name == "Greenland halibut") %>% select(year, pa, length, content, bot.temp)
AP_stof <- stom_spr_good %>% filter(alt.name == "American plaice") %>% select(year, pa, length, content, bot.temp)

# Set up different length bins
ac1 <- AC_stof %>% filter(length > 17 & length <= 45) %>% filter(year != 1984 & year != 1995)
ac2 <- AC_stof %>% filter(length > 45) %>% filter(year != 1984 & year != 1995)
