...
camp_fall_good <- camp_fall %>% filter(lat_dd %in% good_coords$lat_dd & long_dd %in% -good_coords$long_dd) %>% filter(NAFO == "3L")
stom_fall_good <- stom_fall %>% filter(lat_dd %in% good_coords$lat_dd & long_dd %in% -good_coords$long_dd) %>% filter(NAFOdiv == "3L")
...
ac2 <- AC_stof %>% filter(length > 45) %>% filter(year != 1995 & year != 1998)
